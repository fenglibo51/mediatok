...
const checkUpdate = async () => {
  ...
  } catch (e) { updateState.value = 'idle' }
}
    ...
    <div class="fixed top-[calc(env(safe-area-inset-top,0px)+1rem)] bottom-4 left-4 z-50 w-[80%] bg-zinc-900/95 backdrop-blur-2xl rounded-[2.5rem] border border-zinc-700/50 shadow-2xl transition-transform duration-300 ease-out flex flex-col overflow-hidden" :class="isOpen ? 'translate-x-0' : '-translate-x-[110%]'">
      ...
      <div v-if="currentDrawer === 'main'" class="flex flex-col h-full animate-[fadeIn_0.2s_ease-out]">
        ...
        <div class="flex-1 p-6 space-y-4">
          <button @click="currentDrawer = 'servers'" class="w-full p-5 bg-zinc-800/50 rounded-2xl border border-zinc-700/50 flex items-center justify-between group active:scale-95 transition">
        ...
        <div class="p-6 border-t border-zinc-700/30 flex gap-4">
          <button @click="currentDrawer = 'settings'" class="flex-1 py-4 bg-zinc-800 rounded-2xl font-bold text-zinc-300 flex items-center justify-center gap-2 active:scale-95 transition">
          ...
          <button @click="currentDrawer = 'about'" class="w-14 h-14 bg-zinc-800 rounded-2xl flex items-center justify-center text-zinc-300 active:scale-95 transition">
      ...
      <div v-if="currentDrawer === 'servers'" class="flex flex-col h-full animate-[fadeIn_0.2s_ease-out]">
        ...
        <div class="flex-1 p-6 space-y-4">
          <button v-for="s in servers" :key="s.id" @click="goToServerSetup(s.id)" class="w-full p-6 rounded-[2rem] flex items-center justify-between border transition" :class="activeServerId === s.id ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400' : 'bg-zinc-800/40 border-zinc-700 text-zinc-400'">
        ...
        <div class="flex-1 p-8 space-y-6">
          ...
          <div v-if="testState === 'success'" class="space-y-3 py-4">
            ...
            <div class="flex flex-wrap justify-center gap-3">
              <button v-for="lib in availableLibraries" :key="lib.id" @click="toggleLib(lib)" class="px-5 py-2.5 rounded-xl text-xs font-black border transition" :class="selectedLibs.some(l => l.serverId === tempSetupServerId && l.libId === lib.id) ? 'bg-emerald-500 border-emerald-500 text-zinc-950' : 'bg-zinc-800 border-zinc-700 text-zinc-500'">{{ lib.name }}</button>
            </div>
          </div>

          <button @click="testState === 'success' ? saveServerSetup() : testConnection()" class="w-full py-5 rounded-2xl font-black text-lg transition shadow-xl" :class="testState === 'success' ? (hasSelectedLibsForCurrentServer ? 'bg-emerald-500 text-zinc-950' : 'bg-zinc-800 text-zinc-600') : 'bg-zinc-200 text-zinc-900'">
      ...
      <div v-if="currentDrawer === 'settings'" class="flex flex-col h-full animate-[fadeIn_0.2s_ease-out]">
        ...
        <div class="flex-1 p-6 space-y-6">
          ...
          <div class="p-6 bg-zinc-800/40 rounded-3xl border border-zinc-700/30 flex justify-between items-center">
            ...
            <button @click="settings.infinitePlay = !settings.infinitePlay" class="w-14 h-7 rounded-full transition relative" :class="settings.infinitePlay ? 'bg-emerald-500' : 'bg-zinc-600'"><div class="w-5 h-5 bg-white rounded-full absolute top-1 transition-transform" :class="settings.infinitePlay ? 'translate-x-8' : 'translate-x-1'"></div></button>
          </div>
          <div class="p-6 bg-zinc-800/40 rounded-3xl border border-zinc-700/30 flex justify-between items-center">
            <span class="font-bold text-zinc-200">入场默认静音</span>
            <button @click="settings.muteDefault = !settings.muteDefault" class="w-14 h-7 rounded-full transition relative" :class="settings.muteDefault ? 'bg-emerald-500' : 'bg-zinc-600'"><div class="w-5 h-5 bg-white rounded-full absolute top-1 transition-transform" :class="settings.muteDefault ? 'translate-x-8' : 'translate-x-1'"></div></button>
          </div>
        </div>
      </div>
      ...
      <div v-if="currentDrawer === 'about'" class="flex flex-col h-full animate-[fadeIn_0.2s_ease-out]">
        ...
        <div class="flex-1 p-6 space-y-6 overflow-y-auto no-scrollbar text-center">
          <div class="bg-zinc-800/40 rounded-[2.5rem] p-8 border border-zinc-700/30 flex flex-col items-center">
            ...
            <a href="https://github.com/fenglibo51/mediatok" target="_blank" class="flex items-center gap-3 px-6 py-3 bg-zinc-900 rounded-2xl border border-zinc-700/50 hover:bg-zinc-800 transition group">
          ...
          <button @click="checkUpdate" class="w-full py-5 rounded-3xl font-black text-sm transition-all border shadow-lg" :class="updateState === 'idle' ? 'bg-zinc-800 border-zinc-700 text-zinc-300 active:scale-95' : 'bg-emerald-500 border-emerald-400 text-zinc-950'">
            {{ updateState === 'idle' ? '检查并拉取补丁' : updateState === 'checking' ? '正在匹配版本...' : updateState === 'updating' ? '拉取补丁中...' : updateState === 'success' ? '更新部署完成！' : '当前已是最新' }}
          </button>
